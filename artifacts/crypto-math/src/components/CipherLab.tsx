...
import { Copy, Check, Trash2, Download, ArrowRight, RotateCcw, Lock, Hash, Radio, FlipHorizontal, Sparkles, Binary } from "lucide-react";
import { caesarShift, toNumber, fromNumber, toMorse, fromMorse, toSymbol, fromSymbol, toBinary, fromBinary, SYMBOL_MAP } from "../utils/ciphers";
import { recordEncryption, recordDecryption } from "../utils/stats";
...
export default function CipherLab() {
  ...
  const compute = useCallback(() => {
    ...
    let result = "";
    switch (tab) {
      case "caesar":  result = caesarShift(text, actualShift); setSteps(getCaesarSteps(text, actualShift)); break;
      case "number":  result = mode === "encrypt" ? toNumber(text) : fromNumber(text); setSteps([]); break;
      case "morse":   result = mode === "encrypt" ? toMorse(text) : fromMorse(text); setSteps([]); break;
      case "reverse": result = text.split("").reverse().join(""); setSteps([]); break;
      case "symbol":  result = mode === "encrypt" ? toSymbol(text) : fromSymbol(text); setSteps([]); break;
      case "binary":  result = mode === "encrypt" ? toBinary(text) : fromBinary(text); setSteps([]); break;
    }
    setOutput(result);
  ...
  const saveToHistory = () => {
    ...
    setHistory((h) => [{ id: nextId.current++, cipher: label, input, output, time: new Date().toLocaleTimeString() }, ...h].slice(0, 8));
    if (mode === "encrypt" || tab === "reverse") {
      recordEncryption(label);
      showToast(`Encrypted with ${label}!`, "success");
    } else {
      recordDecryption();
      showToast(`Decrypted with ${label}!`, "success");
    }
  };
    ...
    <section id="cipher-lab" className="py-20 px-4 bg-muted/30">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h2 className="text-4xl sm:text-5xl font-black mb-4">Cipher <span className="gradient-text">Lab</span></h2>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">6 different encryption methods — encrypt, decrypt, and explore!</p>
        </div>

        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {TABS.map((t) => (
            <button key={t.id} onClick={() => { setTab(t.id); setOutput(""); setSteps([]); }}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm transition-all ${
                tab === t.id ? `bg-gradient-to-r ${t.color} text-white shadow-md scale-105` : "bg-card border border-border text-muted-foreground hover:border-primary/50"
              }`}>
              {t.icon} {t.label}
            </button>
          ))}
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          <div className="bg-card rounded-2xl p-6 border border-border shadow-sm">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">{activeTab.icon} {activeTab.label} Cipher</h3>
                ...
                {(["encrypt", "decrypt"] as const).map((m) => (
                  <button key={m} onClick={() => setMode(m)}
                    className={`flex-1 py-2 rounded-xl font-bold text-sm transition-all ${
                      mode === m ? `bg-gradient-to-r ${activeTab.color} text-white shadow-md` : "bg-muted text-muted-foreground hover:bg-muted/70"
                    }`}>
            ...
            {tab === "caesar" && (
              <div className="mb-4">
                ...
                <input type="range" min={1} max={25} value={shift} onChange={(e) => setShift(Number(e.target.value))} className="w-full accent-purple-500" />
                <div className="flex justify-between text-xs text-muted-foreground mt-1"><span>1</span><span>25</span></div>
              </div>
            )}
              ...
              <div className="mb-4 bg-muted rounded-xl p-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {Object.entries(SYMBOL_MAP).slice(0, 13).map(([k, v]) => (
                    <span key={k} className="text-xs font-mono bg-card border border-border px-1.5 py-0.5 rounded">{k}={v}</span>
                  ))}
            ...
            {tab === "morse" && (
              <div className="mb-4 bg-muted rounded-xl p-3 text-xs">
                <div className="font-bold text-muted-foreground mb-1">Quick reference:</div>
                <div className="font-mono text-muted-foreground">A=.-  B=-...  S=...  O=---  SOS=... --- ...</div>
              </div>
            )}
            ...
            <div className="mb-4">
              ...
              <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type here..." rows={4}
          ...
          <div className="space-y-4">
            <div className="bg-card rounded-2xl p-6 border border-border shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold">{tab === "reverse" ? "↔️ Reversed" : mode === "encrypt" ? "🔒 Encrypted" : "🔓 Decrypted"} Output</h3>
                <div className="flex gap-2">
                  ...
                  <button onClick={downloadText}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-muted text-muted-foreground hover:bg-muted/70">
            ...
            {output && (
              <div className="bg-card rounded-2xl p-6 border border-border shadow-sm">
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-xs text-muted-foreground text-center">Scan to share the encrypted message</p>
                </div>
              </div>
            )}
          </div>
        ...
        {steps.length > 0 && tab === "caesar" && (
          <div className="bg-card rounded-2xl p-6 border border-border shadow-sm mb-6 animate-fade-in">
            <h3 className="font-bold mb-4">📋 Step-by-Step Letter Changes <span className="text-muted-foreground text-sm font-normal">(shift = {mode === "encrypt" ? shift : 26 - shift})</span></h3>
            <div className="flex flex-wrap gap-2">
              {steps.map((step, i) => (
                <div key={i} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-mono font-bold animate-fade-in ${stepColors[i % stepColors.length]}`} style={{ animationDelay: `${i * 0.04}s` }}>
                  <span>{step.from}</span><ArrowRight className="w-3 h-3" /><span>{step.to}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {history.length > 0 && (
          <div className="bg-card rounded-2xl p-6 border border-border shadow-sm animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setHistory([])} className="text-sm text-muted-foreground hover:text-destructive flex items-center gap-1">
            ...
            <div className="space-y-2">
              {history.map((entry) => (
                <div key={entry.id} className="bg-muted rounded-xl p-3 text-sm flex items-center gap-3 animate-slide-in-up">
                  <span className="bg-card border border-border px-2 py-0.5 rounded-lg text-xs font-bold text-primary shrink-0">{entry.cipher}</span>
